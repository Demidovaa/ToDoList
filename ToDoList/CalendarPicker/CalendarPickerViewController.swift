...
//  ToDoList
//
//  Created by Anastasia Demidova on 04.05.2021.
//
...
private extension CalendarPickerViewController {
    ...
    func monthMetadata(for baseDate: Date) throws -> MonthMetadata {
        ...
        guard let numberOfDaysInMonth = calendar.range(of: .day, in: .month, for: baseDate)?.count,
              let firstDayOfMonth = calendar.date(from: calendar.dateComponents([.year, .month], from: baseDate)) else { throw CalendarDataError.metadataGeneration // 3
        }
    ...
    // 1
    func generateDaysInMonth(for baseDate: Date) -> [DayMetadata] {
        // 2
        ...
        // 3
        var days: [DayMetadata] = (1..<(numberOfDaysInMonth + offsetInInitialRow))
            .map { day in
                // 4
    ...
    // 7
    func generateDay(offsetBy dayOffset: Int, for baseDate: Date, isWithinDisplayedMonth: Bool) -> DayMetadata {
        let date = calendar.date(byAdding: .day, value: dayOffset, to: baseDate) ?? baseDate
        return DayMetadata(date: date, number: dateFormatter.string(from: date), isSelected: calendar.isDate(date, inSameDayAs: selectedDate), isWithinDisplayedMonth: isWithinDisplayedMonth)
    }
    
    // 1
    func generateStartOfNextMonth(using firstDayOfDisplayedMonth: Date) -> [DayMetadata] {
        // 2
        ...
        // 4
        let days: [DayMetadata] = (1...additionalDays).map { generateDay(offsetBy: $0,
                                                                         for: lastDayInMonth,
...
extension CalendarPickerViewController: UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        guard let cell = collectionView.dequeueReusableCell(withReuseIdentifier: CalendarDateCollectionViewCell.reuseIdentifier, for: indexPath) as? CalendarDateCollectionViewCell else { return UICollectionViewCell() }
